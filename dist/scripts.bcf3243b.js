...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"scripts/partials/helperFunctions.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.convertUnixStampToYear = exports.filterDataFromKey = exports.roundToTwoDecimals = void 0;
...
exports.filterDataFromKey = filterDataFromKey;

const convertUnixStampToYear = unix => {
  return new Date(unix * 1000).getFullYear();
};

exports.convertUnixStampToYear = convertUnixStampToYear;
},{}],"scripts/partials/getData.js":[function(require,module,exports) {
"use strict";
...
const getData = (url, key) => {
  return fetchData(url).then(data => {
    if (key) {
      return (0, _helperFunctions.filterDataFromKey)(data, key);
    } else {
      return data;
    }
  });
...
},{"./helperFunctions":"scripts/partials/helperFunctions.js"}],"../node_modules/d3/dist/package.js":[function(require,module,exports) {
"use strict";
...
const createMap = async (amsterdamGeoStreets, amsterdamGeoDistricts) => {
  ...
  const districtGeoData = await (0, _d3HelperFunctions.getData)(amsterdamGeoDistricts);
  const filteredGeoData = amsterdamGeoData.features.filter(e => e.geometry && e.geometry.type !== 'Point' && e.geometry.type !== 'LineString');
  const districtGeoDataFiltered = (0, _d3HelperFunctions.formatDistricts)(Object.entries(districtGeoData).map((_ref) => {
    let [key, value] = _ref;
    return value;
  }));

  _main.svg.call((0, _d.zoom)().on('zoom', e => {
    _main.streets.attr('transform', e.transform);

    _main.districts.attr('transform', e.transform);

    _main.dataPoints.attr('transform', e.transform);
  }));
...
exports.svg = svg;
(0, _createMap.createMap)(amsterdamGeoStreets, amsterdamGeoDistricts);
},{"d3":"../node_modules/d3/index.js","./createMap":"scripts/partials/d3Functions/createMap.js"}],"scripts/partials/d3Functions/plotPoints.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.plotPoints = void 0;
...
const renderGarageData = pointData => {
  const points = _main.dataPoints.selectAll('circle').data(pointData);

  points.attr("cx", d => {
    return (0, _d3HelperFunctions.projection)(d.geometry.coordinates)[0];
  }).attr("cy", d => {
    return (0, _d3HelperFunctions.projection)(d.geometry.coordinates)[1];
  }).attr('data-name', d => d.properties.name).attr('data-capacity', d => d.properties.capacity ? d.properties.capacity : 'Onbekend').attr('data-date', d => (0, _helperFunctions.convertUnixStampToYear)(d.properties.openDate));
  points.enter().append('circle').attr("cx", d => {
    return (0, _d3HelperFunctions.projection)(d.geometry.coordinates)[0];
  }).attr("cy", d => {
    return (0, _d3HelperFunctions.projection)(d.geometry.coordinates)[1];
  }).attr('class', 'parking-garage').attr('data-name', d => d.properties.name).attr('data-capacity', d => d.properties.capacity ? d.properties.capacity : 'Onbekend').attr('data-date', d => (0, _helperFunctions.convertUnixStampToYear)(d.properties.openDate)).on('click', (d, i) => renderGarageInfo(i));
  points.exit().remove();
};
...
const sliderEvent = pointData => {
  _main.slider.on('input', () => {
    updatePointData(_main.slider.node().value, pointData);
  }).on('change', () => {
    updatePointData(_main.slider.node().value, pointData);
  });
};
...
const updatePointData = (year, pointData) => {
  ...
  _main.sliderText.text(selectedYear);

  const filteredPointData = pointData.features.filter(e => e.properties && (0, _helperFunctions.convertUnixStampToYear)(e.properties.openDate) === selectedYear);
...
const resetEvent = pointData => {
  ...
  reset.addEventListener('click', () => {
    _main.sliderText.text('Use the slider to select a year');
...
(0, _getData.getData)('https://npropendata.rdw.nl//parkingdata/v2').then(fetchedGarages => {
  ...
  Promise.all(garagePromises).then(garageArray => {
    ...
    (0, _plotPoints.plotPoints)(garageObject);
  });
});
  ...
  return (0, _getData.getData)(fetchedGarage.staticDataUrl).then(garage => {
    ...
    if (garageData) {
      ...
      return {
        'geometry': {
          'type': 'Point',
          'coordinates': [garageData.accessPointLocation[0].longitude, garageData.accessPointLocation[0].latitude]
        },
        'type': 'Feature',
        'properties': {
...
},{"./partials/getData":"scripts/partials/getData.js","./partials/d3Functions/plotPoints":"scripts/partials/d3Functions/plotPoints.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
